...
def train_and_validate(
    ...
    for epoch in range(configs.train.optimizer.epochs):
        ...
        for data in train_loader:
            ...
            low_f0 = data["low_f0"].unsqueeze(-1)
            high_f0 = data["high_f0"].unsqueeze(-1)
            uv = data["uv"].unsqueeze(-1)

            with autocast(device_type="cuda", enabled=fp16_run):
                # Predict high-frequency F0 contour from generator
                pred_f0, pred_high_f0 = net_g(low_f0, uv, data["style_id"], high_f0)

                # Define mask
                f0_mask = uv != 0.0

                # Masking source and pred F0 contours
                f0 = log_signal_masking_torch(f0, f0_mask)
                high_f0 = log_signal_masking_torch(high_f0, f0_mask)
                pred_f0 = log_signal_masking_torch(pred_f0, f0_mask)
                pred_high_f0 = log_signal_masking_torch(pred_high_f0, f0_mask)

                # Compute discriminator loss
                y_d_hat_r, y_d_hat_g, _, _ = net_d(
                    high_f0.transpose(1, 2),
                    pred_high_f0.transpose(1, 2).detach(),
                )
            ...
            with autocast(device_type="cuda", enabled=fp16_run):
                ...
                y_d_hat_r, y_d_hat_g, fmap_r, fmap_g = net_d(
                    high_f0.transpose(1, 2),
                    pred_high_f0.transpose(1, 2),
                )

            # Compute loss
            high_lf0_l2_loss = torch.sqrt(l2_loss(pred_high_f0, high_f0))
            fm_loss = feature_loss(fmap_r, fmap_g)
            gen_loss, _ = generator_loss(y_d_hat_g)
            loss = high_lf0_l2_loss + fm_loss + gen_loss
            ...
            logging_values = {
                ...
                "loss/l2_high_f0": l2_loss(
                    log_to_constant_trans_torch(pred_high_f0),
                    log_to_constant_trans_torch(high_f0),
                ),
                "loss/l2_high_lf0": high_lf0_l2_loss.item(),
            ...
            if device == 0:
                ...
                if global_step % configs.train.logger.interval_log == 0:
                    print(
                        "Step {} | lr : {:.6f} | time : {} | l2_high_lf0 : {:.5f} | batch/s : {:.2f}".format(
                            global_step,
                            optimizer_g.param_groups[0]["lr"],
                            saver.get_total_time(),
                            logging_values["loss/l2_high_lf0"],
                ...
                if (
                    ...
                    with torch.inference_mode():
                        for data in valid_loader:
                            ...
                            pred_f0, pred_high_f0 = net_g(
                                low_f0, uv, data["style_id"], high_f0
                            )
                            ...
                            # Masking F0 contours
                            f0 = log_signal_masking_torch(f0, f0_mask)
                            low_f0 = log_signal_masking_torch(low_f0, f0_mask)
                            high_f0 = log_signal_masking_torch(high_f0, f0_mask)
                            pred_f0 = log_signal_masking_torch(pred_f0, f0_mask)
                            pred_high_f0 = log_signal_masking_torch(
                                pred_high_f0, f0_mask
                            )

                            # Compute validation loss
                            test_loss += torch.sqrt(l2_loss(pred_f0, f0)).item()

                            # logging tensorboard
                            saver.log_pitch_contour(
                                data["name"][0],
                                data["audio_path"][0],
                                f0.view(-1),
                                low_f0.view(-1),
